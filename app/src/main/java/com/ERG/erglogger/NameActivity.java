...
public class NameActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String suggestedName = Global.devinfo[2].replace(" ","")+"-"+Global.devinfo[3].replace(" ", "") +
                String.format(Locale.getDefault(),
                        "-%4d.%02d.%02d-%02d:%02d.txt",cl.get(Calendar.YEAR), cl.get(Calendar.MONTH)+1,
                        cl.get(Calendar.DAY_OF_MONTH),cl.get(Calendar.HOUR_OF_DAY),cl.get(Calendar.MINUTE));
    ...
    public void nameButtonClick(View button) {
        Global.EXTRA_Name = fileName.getText().toString();
        if (button.getId() == R.id.name_OK_B) {
            Global.SaveFileFlag = true;
        }
